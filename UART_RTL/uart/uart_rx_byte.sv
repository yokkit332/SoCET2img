// uart_rx_byte.sv
// one full RX channel: sync + baud + fsm + shift reg
// used for R/G/B and also for the config line
//
// USE_SHARED_BAUD = 1 when we're hooked up to our own TX (loopback)
// otherwise each channel has its own baud generator
...
module uart_rx_byte #(
    ...
    // only build a baud gen if we need one
    generate
    ...
    // loopback is already same clock domain so skip the sync
